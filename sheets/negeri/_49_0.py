...
TITLE_COORDINATES = [
    {"bm": (2, 3), "en": (3, 3), "is_samb": False},     # Page 1 Titles
...
def populate_jadual_49_0(sheet, hierarchy, report_type):
    ...
    for col_idx, year in COL_YEAR_MAP.items():
        ...
        raw_val = year_data.get("bilangan_taman_asuhan", "n.a")
    ...
    # Dynamic Page Elimination (sheet.delete_rows(start, amount))
    if total_branches <= 20:
        print("     [FORMAT] State only needs 1 page. Eliminating Page 2.")
        sheet.delete_rows(62, 238) # Deletes ~238 rows starting at 62